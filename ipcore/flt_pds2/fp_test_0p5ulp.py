...
def fp_test(op_sel, prec, result_prec, int_type, line, b2f, f2b, np_2f, binary_is_denorm):
    global cnt
    cnt = cnt + 1
    global sum
    # if op_sel == "0": # abs, too simple, we don't test it
    if op_sel == '1' or op_sel == '17' : # accum
        LSB = -31
        ...
        if golden_float == 0 and result_float == 0: # in case that binary of -0 and +0 unequal
            golden_minus_result = 0
        elif np.isnan(golden_float) and np.isnan(result_float):
            golden_minus_result = 0
        else:
            golden_minus_result = int(golden_binary, 2) - int(result_binary, 2)
        if golden_minus_result != 0:
        ...
        if operation_binary == "00000100": # unordered
            ...
            golden_bool = a_float >= b_float
        elif operation_binary == "00111100": # condition code
            golden_unordered = np.isnan(a_float) or np.isnan(b_float)
            golden_gt = a_float > b_float
            golden_lt = a_float < b_float
            golden_eq = a_float == b_float
            golden_bool = 8*(golden_unordered) + 4*(golden_gt) + 2*(golden_lt) + golden_eq
        ...
        if result_prec == '1': # single
            result_float = binary_to_float(result_binary)
            golden_float = np.single(float(a_int_uint))
        else: # double
            result_float = binary_to_double(result_binary)
    ...
    #        golden_minus_result = int(golden_binary, 2) - int(result_binary, 2)
    #    if golden_minus_result != 0:
    #        print("{:10}, {:15}, {:25}, {:25}, {:25}, {:25}, {:25}".format(cnt, golden_minus_result, golden_float, result_float, a_float, b_float, c_float))
    elif op_sel == '10': # log
        a_binary, result_binary = line.split()
        if binary_is_denorm(a_binary):
            a_float = 0.0
        ...
        if prec == '2': # double
            a = 1 + int(a_binary[12:17], 2) * (2**(-5)) + (2**(-6))
            if a_binary[11:12] == '1':
                a_exp_even_odd = 1
        ...
        golden_taylor_frac = 2 * (a0 - a1 * x_minus_a + a2 * (x_minus_a**2) - a3 * (x_minus_a**3) + a4 * (x_minus_a**4) - a5 * (x_minus_a**5))
        ...
        if golden_minus_result != 0:
            print("{:10}, {:15}, {:15}, {:25}, {:25}, {:25}, {:25}".format(cnt, golden_minus_result, golden_taylor_minus_result, golden_float, golden_taylor_float, result_float, a_float))
    elif op_sel == '14': # sqrt
        a_binary, result_binary = line.split()
        if binary_is_denorm(a_binary):
            a_float = 0.0
        ...
        if binary_is_denorm(c_binary):
            ...
            if operation_binary == "00000000":
                ...
                c_float = -b2f(c_binary)
        result_float = b2f(result_binary)
        a_mul_b_float = np_2f(a_float * b_float)
        golden_float = np_2f(a_mul_b_float + c_float)
        golden_binary = f2b(golden_float)
        if np.isnan(golden_float) and np.isnan(result_float):
            golden_minus_result = 0
        elif golden_float == 0 and result_float == 0: # in case that binary of -0 and +0 unequal
    ...
    elif op_sel == '16' : # umacc
        a_tlast, operation_binary, b_binary, a_binary, result_tlast, result_binary = line.split()
        if binary_is_denorm(a_binary):
            a_float = 0.0
        else:
            a_float = b2f(a_binary)
        ...
        a_mul_b_float = np_2f(a_float * b_float)
        if operation_binary == "00000000":
            sum = sum + a_mul_b_float
        elif operation_binary == "00000001":
            sum = sum - a_mul_b_float
        golden_float = np_2f(sum)
        golden_binary = f2b(golden_float)
        if binary_is_denorm(golden_binary):
            golden_float = 0.0
        if np.isnan(a_float):
            sum = float('nan')
        elif np.isinf(a_mul_b_float):
            sum = a_mul_b_float
        if a_tlast == "1":
            sum = 0
        if golden_float == 0 and result_float == 0: # in case that binary of -0 and +0 unequal
        ...
        if a_tlast == "1" and result_tlast == "1":
            ...
            a_tlast_minus_result_tlast = -1
        if golden_minus_result != 0:
            print("{:10}, {:5}, {:15}, {:25}, {:25}, {:25}, {:25}, {:10}".format(cnt, a_tlast_minus_result_tlast, golden_minus_result, golden_float, result_float, a_float, b_float, operation_binary))

if __name__ == '__main__':
    if sys.argv[1] == "1" or sys.argv[1] == "17": # accum
        ...
        print("{:^10}  {:^15}  {:^25}  {:^25}  {:^25}".format("cnt", "diff", "golden_float", "result_float", "a_float"))
    elif sys.argv[1] == '15': # fma
        print("{:^10}  {:^15}  {:^25}  {:^25}  {:^25}  {:^25}  {:^25}".format("cnt", "diff", "golden_float", "result_float", "a_float", "b_float", "c_float", "op_binary"))
    elif sys.argv[1] == "16": # umacc
        print("{:^10}  {:^5}  {:^15}  {:^25}  {:^25}  {:^25}  {:^10}".format("cnt", "tlast_diff", "diff", "golden_float", "result_float", "a_float", "b_float", "op_binary"))
    for line in sys.stdin:
        if line == '': # If empty string is read then stop the loop
            break
        if len(sys.argv) == 5:
            if sys.argv[2] == '0': # half
                fp_test(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], line, binary_to_half, float_to_binary16, np.half, binary16_is_denorm)
            elif sys.argv[2] == '1': # single
                fp_test(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], line, binary_to_float, float_to_binary32, np.single, binary32_is_denorm)
            else: # double
                fp_test(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], line, binary_to_double, float_to_binary64, np.double, binary64_is_denorm)